...
if(isset($_POST['s_upload'])) { 
	if(strlen($_FILES['uploadedfile']['tmp_name'])) {
		...
		$target_path = $target_path . 'export_orase.xls';
		unlink($target_path);
...
$data->read(SITE_DIR."admin/excel/export_orase.xls");
...
if(!empty($upd_arr) )   $valori=implode(', ',$upd_arr);
 echo  $whr;
// echo  $valori;  
 if(  $whr<>'') $updi=mysql_query   ( "INSERT INTO  erad_orase set id_oras={$whr} ");
 if(  $whr<>'') $upd=mysql_query   ( "UPDATE  erad_orase set {$valori} WHERE {$whr} ");
...
<table width="100%" cellpadding="0" cellspacing="0" border="0"   style="border:0; background-image:url(img/top.jpg); background-repeat:repeat-x;"  >
   <tr>
	 ...
	 <div style="text-align:center;   float:left">&nbsp;Export / Import actualizare orase </div>
     </td>
   </tr>
</table>
...
<form action="<?=$src?>?i=1" method="POST" name="234" id="432432aa" enctype="multipart/form-data">
	  ...
	  <tr>
		<td align="center"> Importa lista oraselor </td>
	  </tr>
	  <tr>
