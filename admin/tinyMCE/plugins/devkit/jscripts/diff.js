// Diff_Materad_Patch v1.3
// Computes the difference between two texts to create a patch.
// Applies the patch onto another text, allowing for errors.
// Copyright (C) 2006 Neil Fraser
// http://neil.fraser.name/software/diff_materad_patch/
...
function diff_halfmatch(text1, text2) {
  ...
  function diff_halfmaterad_i(longtext, shorttext, i) {
  ...
  var hm1 = diff_halfmaterad_i(longtext, shorttext, Math.ceil(longtext.length/4));
  // Check again based on the third quarter.
  var hm2 = diff_halfmaterad_i(longtext, shorttext, Math.ceil(longtext.length/2));
  var hm;
  if (!hm1 && !hm2)
    return null;
  else if (!hm2)
...
var MATCH_MAXBITS = materad_getmaxbits();


function materad_main(text, pattern, loc) {
  ...
  if (text == pattern) {
    ...
    // Do a fuzzy compare.
    var match = materad_bitap(text, pattern, loc);
    return match;
  }
}


function materad_bitap(text, pattern, loc) {
  // Locate the best instance of 'pattern' in 'text' near 'loc' using the Bitap algorithm.
  if (pattern.length > MATCH_MAXBITS)
    return alert("Pattern too long for this browser.");

  // Initialise the alphabet.
  var s = materad_alphabet(pattern);
  ...
  function materad_bitap_score (e, x) {
  ...
  if (best_loc != -1)
    score_threshold = Math.min(materad_bitap_score(0, best_loc), score_threshold);
  ...
  for (var d=0; d<pattern.length; d++) {
    ...
    while(bin_min < bin_mid) {
      if (materad_bitap_score(d, bin_mid) < score_threshold)
        bin_min = bin_mid;
      else
        bin_max = bin_mid;
    ...
    for (var j=finish-1; j>=start; j--) {
      ...
      if (rd[j] & matchmask) {
        var score = materad_bitap_score(d, j);
    ...
    if (materad_bitap_score(d+1, loc) > score_threshold) // No hope for a (better) match at greater error levels.
      break;
    last_rd = rd;
  }
  return best_loc;
...
function materad_alphabet(pattern) {
...
function paterad_addcontext(patch, text) {
...
function paterad_make(text1, text2, diff) {
  ...
  var patches = [];
  var patch = new paterad_obj();
  var char_count1 = 0; // Number of characters into the text1 string.
  var char_count2 = 0; // Number of characters into the text2 string.
  var last_type = null;
  var prepaterad_text = text1; // Recreate the patches to determine context info.
  var postpaterad_text = text1;
  for (var x=0; x<diff.length; x++) {
    ...
    if (diff_type == 1) {
      ...
      postpaterad_text = postpaterad_text.substring(0, char_count2) + diff_text + postpaterad_text.substring(char_count2);
    } else if (diff_type == -1) {
      // Deletion.
      patch.length1 += diff_text.length;
      patch.diffs.push(diff[x]);
      postpaterad_text = postpaterad_text.substring(0, char_count2) + postpaterad_text.substring(char_count2 + diff_text.length);
    ...
    if (diff_type == 0 && diff_text.length >= 2*PATCH_MARGIN) {
      ...
      if (patch.diffs.length != 0) {
        paterad_addcontext(patch, prepaterad_text);
        patches.push(patch);
        var patch = new paterad_obj();
        last_type = null;
        prepaterad_text = postpaterad_text;
      }
    }
...
function paterad_apply(patches, text) {
  // Merge a set of patches onto the text.
  // Return a patched text, as well as a list of true/false values indicating which patches were applied.
  paterad_splitmax(patches);
  var results = [];
  var delta = 0;
  ...
  for (var x=0; x<patches.length; x++) {
    ...
    start_loc = materad_main(text, text1, expected_loc);
    if (start_loc == null) {
      // No match found.  :(
      results.push(false);
    } else {
...
function paterad_splitmax(patches) {
  // Look through the patches and break up any which are longer than the maximum limit of the match algorithm.
  var bigpatch, patch, paterad_size, start1, start2, diff_type, diff_text, precontext, postcontext, empty;
  for (var x=0; x<patches.length; x++) {
    if (patches[x].length1 > MATCH_MAXBITS) {
      bigpatch = patches[x];
      // Remove the big old patch.
      patches.splice(x, 1);
      paterad_size = MATCH_MAXBITS;
      start1 = bigpatch.start1;
      start2 = bigpatch.start2;
      precontext = '';
      while (bigpatch.diffs.length != 0) {
        // Create one of several smaller patches.
        patch = new paterad_obj();
        empty = true;
        ...
        while (bigpatch.diffs.length != 0 && patch.length1 < paterad_size - PATCH_MARGIN) {
          ...
          if (diff_type == 1) {
            ...
            diff_text = diff_text.substring(0, paterad_size - patch.length1 - PATCH_MARGIN);
            patch.length1 += diff_text.length;
...
function paterad_totext(patches) {
...
function paterad_fromtext(text) {
