...
if (isset($vl[activ])) $vl[activ]=1; else $vl[activ]=0;
	...
	if(empty($error)) {
$ins = $Usr->update($vldb, $id_camp);
 
	mysql_query("delete from erad_campuri_categorii where id_camp='".$id_camp."'");
...
<table width="95%" align="center" cellpadding="5" cellspacing="1" bgcolor="#cccccc">
 ...
 <tr>
   ...
   <form name="<?=$form_name?>" action="<?=$scr?>?section=<?=$section?>&id_camp=<?=$id_camp?>" method="post" onSubmit="return validate_form_<?=$form_name?> ( );">
<fieldset class="">
...
<table width="100%" align="center" cellpadding="5" cellspacing="1" bgcolor="#cccccc">
 ...
 <tr>
  ...
  <input type="text"  id="denumire_camp" name="denumire_camp" size="50"   value="<?=$vl[denumire_camp]?>">  </td>
 </tr>
...
<input type="button" value="Adauga" class="but" onclick="load_categorii(categ.options[categ.selectedIndex].value,0, 'add')" />	   </td>
