...
$Usr = new UserManagement($DBF['erad_campuri_valori']);  
	...
	if(empty($error)) {
$ins = $Usr->update($vldb, $id_valoare);
...
<form name="<?=$form_name?>" action="<?=$scr?>?section=<?=$section?>&id_valoare=<?=$id_valoare?>" method="post"  onSubmit="return validate_form_<?=$form_name?> ( );">
<table width="95%" cellpadding="5" cellspacing="1" bgcolor="#B5C6CF" align="center">
 ...
 <?    $campuri = mysql_query_assoc("select * from erad_campuri order by denumire_camp");?>
	...
	?>
	<option value="<?=$mnu[id_camp]?>" <?=selected($mnu[id_camp], $vl[id_camp]);?>><?=$mnu[denumire_camp]?>  </option>
	<? }?>
	</select>   </td>
 </tr>
 <tr>
  <td width="13%" align="right" bgcolor="#ffffff"><strong>Valoare    </strong></td>
  <td bgcolor="#ffffff"><input type="text" name="valoare_camp" size="70" maxlength="255" value="<?=$vl[valoare_camp]?>"></td>
 </tr>
 <tr>
