...
$Usr = new UserManagement($DBF['erad_curier']);  
	...
	if(empty($error)) {
$ins = $Usr->update($vldb, $id_curier);

		if($ins) {
mysql_query ("delete from erad_judete_curier where id_curier='".$id_curier."'");		
 		 ...
 		 echo js_redirect($scr.'?section='.$mnp1.'_3&id_curier='.$id_curier.'&msg=Datele au fost modificate');
		}
	}
}
$Usr = new UserManagement($DBF['erad_curier']);  

$vldb=$Usr->get01($id_curier);
...
<form name="<?=$form_name?>" action="<?=$scr?>?section=<?=$section?>&id_curier=<?=$id_curier?>"   method="post"  onSubmit="return validate_form_<?=$form_name?> ( );">
...
<table width="95%" cellpadding="5" cellspacing="1" bgcolor="#B5C6CF" align="center">
 ...
 <tr>
   ...
   $prt=mysql_query_assoc("select * from erad_judete_curier where id_curier='".$id_curier."' and id_judet='".$judete[$j][id_judet]."' "); 
   ?>
  ...
  <tr>
    ...
    <td width="115" bgcolor="#FFFFFF"><input type="text"   name="taxa_standard[<?=$judete[$j][id_judet]?>]" size="10" maxlength="255" value="<?=$prt[0][taxa_standard]?>" />
lei</td>
    <td width="3" bgcolor="#999999">&nbsp;</td>
    </tr>
  <tr>
    ...
    <td nowrap="nowrap" bgcolor="#FFFFFF"><input type="text"    name="taxa_per_kg[<?=$judete[$j][id_judet]?>]" size="10" maxlength="255" value="<?=$prt[0][taxa_per_kg]?>" />
lei/kg</td>
  ...
  <tr>
    ...
    <input type="text"   name="suma_transport_gratuit[<?=$judete[$j][id_judet]?>]" size="10" maxlength="255" value="<?=$prt[0][suma_transport_gratuit]?>" />
lei
    </td>
    <td nowrap="nowrap" bgcolor="#999999">&nbsp;</td>
  </tr>
