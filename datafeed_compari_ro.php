...
left join erad_brands on erad_produse.id_brand=erad_brands.id_brand
...
$pic = mysql_query_assoc(" select * from erad_galerie where id_produs='".$pp[id_produs]."' order by prim desc");	
 ...
 ?><?=$categorie?> | <?=$pp[denumire_brand]?> |  | <?=$pp[produs_cod]?> | <?=$pp[produs]?> | <?=$prod_desc?> | <?=get_link_produs($pp[id_produs], $pp[produs])?> | <? if(is_file(PICS_DIR_LARGE.$pic[0][pic])) {?><?=PICS_URL_THUMB.$pic[0][pic]?><? }?> | <? if($pp[pret_oferta]>0) { if($pp[id_moneda]==0) echo $pp[pret_oferta]; else echo fx($pp[pret_oferta]*curs_valoare($pp[id_moneda])); } else { if($pp[id_moneda]==0) echo $pp[pret]; else echo fx($pp[pret]*curs_valoare($pp[id_moneda])); }?> | RON |
 <? }?><?
