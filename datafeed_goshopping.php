...
$pic = mysql_query_assoc(" select * from erad_galerie where id_produs='".$pp[id_produs]."' order by prim desc");	
 ...
 ?><?=$categorie?>|||<?=$pp[produs_cod]?>|<?=$pp[produs]?>|<?=$prod_desc?>|<?=get_link_produs($pp[id_produs], $pp[produs],$pp[link])?>|<? if(is_file(PICS_DIR_LARGE.$pic[0][pic])) {?><?=PICS_URL_MEDIU.$pic[0][pic]?><? }?>|<? if($pp[pret_oferta]>0) echo $pp[pret_oferta]; else echo $pp[pret];?>|RON 
 <? }?><?
