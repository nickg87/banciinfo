...
if(isset($_POST['s_upload'])) { 
	if(strlen($_FILES['uploadedfile']['tmp_name'])) {
		...
		$target_path = $target_path . 'export.xls';
		unlink($target_path);
...
$data->read(SITE_DIR."admin/excel/export.xls");
...
if(!empty($upd_arr) )   $valori=implode(', ',$upd_arr);
 //echo  $whr;
 if(  $whr<>'') $upd=mysql_query   ( "UPDATE  erad_produse set {$valori} WHERE {$whr} ");
//echo '<hr>';
...
<table width="100%" cellpadding="0" cellspacing="0" border="0"   style="border:0; background-image:url(img/top.jpg); background-repeat:repeat-x;"  >
   <tr>
	 ...
	 <div style="text-align:center;   float:left">&nbsp;Export / Import actualizare preturi </div>
     </td>
   </tr>
</table>
