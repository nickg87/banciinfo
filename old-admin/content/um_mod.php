...
$Usr = new UserManagement($DBF['erad_um']);  
	...
	if(empty($error)) {
$ins = $Usr->update($vldb, $id_um);
 
		if($ins) {
		update_order('erad_um', 'id_um', 'ord', "");
 			echo js_redirect($scr.'?section='.$mnp1.'_6&msg=Modificare efectuata.');
		}
	}
}

$Usr = new UserManagement($DBF['erad_um']);  
$vldb=$Usr->get01($id_um);
...
<table width="95%" align="center" cellpadding="5" cellspacing="1" bgcolor="#cccccc">
 ...
 <tr>
   ...
   <form name="<?=$form_name?>" action="<?=$scr?>?section=<?=$section?>&id_um=<?=$id_um?>" method="post" onSubmit="return validate_form_<?=$form_name?> ( );">
<fieldset class="">
...
<table width="100%" align="center" cellpadding="5" cellspacing="1" bgcolor="#cccccc">
 ...
 <tr>
  ...
  <td bgcolor="#ffffff">
  <input type="text"  id="um" name="um" size="50"   value="<?=$vl[um]?>">  </td>
 </tr>
 <tr>
