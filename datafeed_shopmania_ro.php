...
function alege_moneda($id_moneda) {
if($id_moneda==0) return 'RON';
if($id_moneda==1) return 'EUR';
if($id_moneda==2) return 'USD';
...
$pic = mysql_query_assoc(" select * from erad_galerie where id_produs='".$pp[id_produs]."' order by prim desc");	
 ...
 ?><?=$categorie?> | <?=$pp[denumire_brand]?> |  | <?=$pp[produs_cod]?> <?=$pp[id_produs]?> | <?=$pp[produs]?> | <?=$prod_desc?> | <?=get_link_produs($pp[id_produs], $pp[produs])?> | <? if(is_file(PICS_DIR_LARGE.$pic[0][pic])) {?><?=PICS_URL_MEDIU.$pic[0][pic]?><? }?> | <? if($pp[pret_oferta]>0) echo $pp[pret_oferta]; else echo $pp[pret];?> | <?=alege_moneda($pp[id_moneda])?> |
 <? }?><?
