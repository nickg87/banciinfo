...
$pic = mysql_query_assoc(" select * from erad_galerie where id_produs='".$pp[id_produs]."' order by prim desc");	
 ...
 ?><?=substr($pp[link],0,25)?> | <?=$pp[denumire_brand]?> |  | <?=$pp[produs_cod]?> <?=$pp[id_produs]?> | <?=substr($pp[produs],0,60)?> | <?=$prod_desc?> | <?=get_link_produs($pp[id_produs], $pp[produs])?> | <? if(is_file(PICS_DIR_LARGE.$pic[0][pic])) {?><?=PICS_URL_MEDIU.$pic[0][pic]?><? }?> | <? if($pp[pret_oferta]>0) echo $pp[pret_oferta]; else echo $pp[pret];?> | RON |
 <? }?><?
