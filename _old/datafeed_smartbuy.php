...
$pic = mysql_query_assoc(" select * from erad_galerie where id_produs='".$pp[id_produs]."' order by prim desc");	
  $prod_desc = replace_not_in_tags("\n", "<BR />", $pp[descriere]);
	 ...
	 if($pp[pret_oferta]>0) $pret= $pp[pret_oferta]; else $pret= $pp[pret];
 
 if($pp[id_moneda]==0) $pret=$pret; else $pret=  fx($pret*curs_valoare($pp[id_moneda]));	
		
 ?> <?=$pp[produs_cod]?> <?=$pp[id_produs]?> | <?=$categorie?> | <?=$pp[denumire_brand]?> | <?=$pp[produs]?> | <?=$prod_desc?> | <?=$pret?> | RON | | | <?=get_link_produs($pp[id_produs], $pp[produs])?> | <? if(is_file(PICS_DIR_LARGE.$pic[0][pic])) {?><?=PICS_URL_MEDIU.$pic[0][pic]?><? }?> |  |  | |
 <? }?><?
