...
if (isset($vl[activ])) $vl[activ]=1; else $vl[activ]=0;
	...
	if(empty($error) and strlen($_FILES['pic']['tmp_name'])) {
$ins = $Usr->insert($vldb);

$id_promotie=get_last_mysql_id('erad_promotii');
if ($ins and strlen($_FILES['pic']['tmp_name']))	 
$u = upload_poza2('pic', PICS_DIR, PICS_DIR_PROMO, PICS_PROMO, BANNER_PROMO, 'erad_promotii', 'pic', 'id_promotie', 'promo'.$id_promotie);
...
if(is_file(PICS_DIR.$q[0][pic])) 	unlink(PICS_DIR . $q[0][pic]);  
	...
	for($i = 1; $i <= 4; $i++) {
			...
			//	upload_poza2('pic'.$i, PICS_DIR_SIZE1, PICS_DIR_SIZE2, PICS_SIZE1, PICS_SIZE2, 'valus_imobile', 'pic'.$i, 'id_imobil', 'pic'.$i);
				$u = upload_poza2('poza'.$i,  PICS_DIR, PICS_DIR_PROMO, PICS_PROMO, PICS_PROMO, 'erad_promotii', 'poza'.$i, 'id_promotie', $n_poza.$i);
 	$q=mysql_query_assoc("select * from erad_promotii where id_promotie='".$id_promotie."'");
...
<table width="95%" align="center" cellpadding="5" cellspacing="1" bgcolor="#cccccc">
 ...
 <tr>
  <td align="right" width="12%" bgcolor="#ffffff">Nume oferta :</td>
  <td width="88%" bgcolor="#ffffff">
  <input type="text"  name="nume_promotie" size="50" maxlength="255" value="<?=$vl[nume_promotie]?>"></td>
 </tr><tr>
   <td align="right" bgcolor="#ffffff">Banner:       </td>
   <td bgcolor="#ffffff">
   <input type="file" name="pic" size="20"> <font style="font-size:9px; font-family:tahoma;">Dimensiuni maxime:<strong>  <?=BANNER_PROMO?> x <?=BANNER_PROMO_H?>  pixeli</strong>)</font>   </td>
 </tr>
 <tr>
   <td align="right" bgcolor="#ffffff">Link:</td>
